(function(a){a.slider=function(b,d){var c={setting:{imgAry:[],hasDot:true,isLoop:true,indexNow:0},init:function(f,g){var e=this;this.$el=null;this.$close=null;
...
this.moveX=0;this.moveY=0;this.scale=1;this.render();},onload:function(){this.loadNum++;if(this.loadNum==this.liLength){this.$loading.remove();this.initImg();
this.$ul.show();this.onEvent();if(d){d();}}},render:function(){var g=this;var f='<div class="slide-wrapper" id="J-slide-wrapper"><span class="slide-tips" id="J-slide-load">加载中...</span><ul class="slide"></ul><ul class="slide-nav"></ul><span class="slide-close" id="J-slide-colse">关闭</span></div>';
this.$el=a(f);this.$close=this.$el.find("#J-slide-colse");this.$ul=this.$el.find(".slide");this.$nav=this.$el.find(".slide-nav");this.$loading=this.$el.find("#J-slide-load");
this.liWidth=this.winWidth;this.ulWidth=this.setting.isLoop?this.liWidth*(this.liLength+2):this.liWidth*this.liLength;a("body").append(this.$el);this.appendImg();
if(this.setting.hasDot){for(var e=0;e<this.liLength;e++){this.$nav.append("<li></li>");}this.$navli=this.$el.find(".slide-nav li");this.$navli.eq(0).addClass("on");
}this.$el.css({width:this.winWidth,height:this.winHeight}).show();this.$ul.css({width:this.ulWidth+"px"});if(this.setting.isLoop){this.$ul.css("left",-this.liWidth+"px");
...
}this.$li=this.$el.find(".slide li");this.$li.css({width:this.liWidth+"px"});},appendImg:function(){var f="",g=this;for(var e=0;e<this.liLength;e++){if(e==0&&this.setting.isLoop){f+='<li class="Route"><img src="'+this.setting.imgAry[this.liLength-1]+'" /></li>';
}f+='<li class="Route"><img src="'+this.setting.imgAry[e]+'" /></li>';if(e==this.liLength-1&&this.setting.isLoop){f+='<li class="Route"><img src="'+this.setting.imgAry[0]+'" /></li>';
}}this.$ul.append(f);this.$imgs=this.$ul.find("img");this.$imgs.on("load error emptied stalled",function(h){g.onload.call(g,h);});},initImg:function(){var k=this.setting.isLoop?this.liLength+2:this.liLength;
for(var j=0;j<k;j++){var h=this.$imgs.eq(j);var e=h[0].naturalHeight;var f=h[0].naturalWidth;if(!(f&&e)){h.parent().html('<span class="slide-tips">加载失败</span>');
}else{if(e>f){var l=this.winHeight/e;var g=this.winWidth/f;if(g*e>this.winHeight){h.css({width:f*l,height:this.winHeight});}else{h.css({width:this.winWidth,height:e*g});
}}}}},onEvent:function(){var e=this;this.$el.on("touchstart doubleTap",function(f){e.onTouchEvent.call(e,f);});},onTouchEvent:function(j){var h=this,g=j.type,i=j.touches||[],l=this.setting.isLoop?this.$li.eq(this.indexNow+1):this.$li.eq(this.indexNow),k=0;
if(j.preventDefault){j.preventDefault();}if(a(j.target).attr("id")==="J-slide-colse"){this.destory();return;}switch(g){case"touchstart":console.log("touchstart");
if(i.length===2){this.initFingerDis=this.fingersDistance(i);this.isScale=true;}else{if(i.length===1&&this.isScale&&!this.isMove){this.initImgMoveX=i[0].clientX-this.moveX;
this.initImgMoveY=i[0].clientY-this.moveY;}else{if(i.length===1&&!this.isScale){this.initSlideX=this.lastSlideX=i[0].clientX;this.moveLength=0;}}}this.$el.on("touchmove touchend",function(m){h.onTouchEvent.call(h,m);
});break;case"touchmove":console.log("touchmove");if(i.length===2&&!this.isMove){this.lastFingerDis=this.fingersDistance(i);var f=this.lastFingerDis/this.initFingerDis;
...
}}}break;case"touchend":console.log("touchend");this.$el.off("touchmove touchend");this.resetSliderPosition();this.resetImgPosition(l);this.$el.off("touchmove touchend");
...
}var e=this.moveLength;var f=this.indexNow!=0||this.setting.isLoop,g=this.indexNow!=this.liLength-1||this.setting.isLoop;if(e<0&&Math.abs(e)>this.liWidth/3&&g){console.log("moveToLeft");
this.movenext();}else{if(e>0&&Math.abs(e)>this.liWidth/3&&f){console.log("moveToRight");this.moveprev();}else{this.transform(3,-(this.indexNow)*this.liWidth,0,1,"auto",this.$ul);
